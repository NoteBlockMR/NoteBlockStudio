function draw_window_midi_import() {
	...
	draw_text_dynamic(x1 + 260, y1 + 52, localize_ko("Max. channel height:"))
	popup_set_window(x1 + 260, y1 + 52, 140, 16, localize_ko("The maximum allowed layers per channel.\nClick and drag to adjust."))
	w_midi_maxheight = median(1, draw_dragvalue(1, x1 + 380, y1 + 52, w_midi_maxheight, 1), 20)
	if (draw_checkbox(x1 + 260, y1 + 72, w_midi_octave, localize_ko("Keep within octave range"), localize_ko("Whether to automatically transpose the notes\nto keep them within the 2 octave range.")) && wmenu = 0) w_midi_octave=!w_midi_octave
	if (draw_checkbox(x1 + 260, y1 + 92, w_midi_vel, localize_ko("Read note velocity"), localize_ko("Whether to copy the volume data found\nin each MIDI note.")) && wmenu = 0) w_midi_vel=!w_midi_vel
	...
	if (draw_button2(x1 + 520 - 80, y1 + 368, 72, localize_ko("Cancel")) && wmenu = 0 && (windowopen = 1 || theme != 3)) {midifile = "" w_midi_tab = 0 windowclose = 1}
	} else {
	draw_theme_font(font_main_bold)
	...
	if (draw_button2(x1 + 520 - 160, y1 + 368, 72, condstr(language != 1, localize_ko("Use default"), "使用默认值")) && wmenu = 0) {
