function draw_window_set_accent() {
	...
	draw_theme_color()
	if (draw_button2(x1 + 10, y1 + 408, 72, condstr(language != 1, localize_ko("Use default"), "使用默认值"), false, true) && wmenu = 0) {
	    if ((language == 0 && question(localize_ko("Are you sure?"), localize_ko("Confirm"))) || (language == 1 && question("你确定吗？", "确定"))) {
			accent1 = 0
			accent2 = 120
			accent3 = 215
			draw_accent_init()
	...
	if (draw_button2(x1 + 104, y1 + 408, 72, condstr(language != 1, localize_ko("Cancel"), "取消"), false, true)) {window = w_preferences}
