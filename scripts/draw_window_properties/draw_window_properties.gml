function draw_window_properties() {
	...
	if (window != prevwindow || songs[song].loopend = 0) {
	...
	if (language != 1) draw_text_dynamic(x1 + 8, y1 + 8, localize_ko("Song Properties"))
	...
	if (language != 1) draw_areaheader(x1 + 22, y1 + 48, 396, 200, localize_ko("Information"))
	else draw_areaheader(x1 + 22, y1 + 48, 396, 200, "信息")

	if (language != 1) draw_text_dynamic(x1 + 32, y1 + 67, localize_ko("Song name:"))
	else draw_text_dynamic(x1 + 32, y1 + 67, "歌曲名:")
	a = cursong.song_name
	if (language != 1) cursong.song_name = draw_inputbox(1, x1 + 158, y1 + 64, 240, cursong.song_name, localize_ko("The name of the song."))
	else cursong.song_name = draw_inputbox(1, x1 + 158, y1 + 64, 240, cursong.song_name, "歌曲的名称。")
	if (cursong.song_name = "") {
	    draw_set_color(c_gray)
	    if (language != 1) draw_text_dynamic(x1 + 161, y1 + 68, localize_ko("Untitled song"))
	    else draw_text_dynamic(x1 + 161, y1 + 68, "无名歌曲")
	    draw_theme_color()
	}
	if (a != cursong.song_name) cursong.changed = 1

	if (language != 1) draw_text_dynamic(x1 + 32, y1 + 67 + 23, localize_ko("Song author:"))
	else draw_text_dynamic(x1 + 32, y1 + 67 + 23, "作者:")
	a = cursong.song_author
	if (language != 1) cursong.song_author = draw_inputbox(2, x1 + 158, y1 + 64 + 23, 240, cursong.song_author, localize_ko("The name of the creator of the song."))
	...
	if (language != 1) draw_text_dynamic(x1 + 32, y1 + 67 + 23 * 2, localize_ko("Original song author:"))
	else draw_text_dynamic(x1 + 32, y1 + 67 + 23 * 2, "原作者:")
	a = cursong.song_orauthor
	if (language != 1) cursong.song_orauthor = draw_inputbox(3, x1 + 158, y1 + 64 + 23 * 2, 240, cursong.song_orauthor, localize_ko("The name of the creator of the original song\n(Leave empty if you composed the song yourself.)"))
	...
	if (language != 1) draw_text_dynamic(x1 + 32, y1 + 67 + 23 * 3, localize_ko("Description:"))
	else draw_text_dynamic(x1 + 32, y1 + 67 + 23 * 3, "简介:")
	a = cursong.song_desc
	if (language != 1) cursong.song_desc = draw_textarea(4, x1 + 158, y1 + 64 + 23 * 3, 240, 100, cursong.song_desc, localize_ko("Enter a description for your song."))
	...
	draw_theme_color()
	if (language != 1) draw_areaheader(x1 + 22, y1 + 268, 396, 105, localize_ko("Playback"))
	else draw_areaheader(x1 + 22, y1 + 268, 396, 85, "播放")

	if (language != 1) draw_text_dynamic(x1 + 37, y1 + 285, localize_ko("Time signature:"))
	...
	if (language != 1) popup_set_window(x1 + 37, y1 + 283, 110, 18, localize_ko("The time signature of the song."))
	else popup_set_window(x1 + 37, y1 + 283, 110, 18, "乐曲的拍号。")

	a = cursong.loop
	if (language != 1) {if (draw_checkbox(x1 + 252, y1 + 285, cursong.loop, localize_ko("Enable looping"), localize_ko("Whether to loop this song back to")+br+localize_ko("the start at the end of playback."), false, true)) cursong.loop=!cursong.loop}
	...
	if (language != 1) draw_text_dynamic(x1 + 252, y1 + 305, localize_ko("Loop start tick:"))
	...
	// Loop end tick
	if (!cursong.loop) draw_set_color(c_gray)
	if (language != 1) draw_text_dynamic(x1 + 252, y1 + 325, localize_ko("Loop end tick:"))
	...
	if (cursong.loop && cursong.loopend > cursong.enda + 1) {
		...
		if (language != 1) draw_text_dynamic(x1 + 40 + (theme != 3) * 2, y1 + 390 + (theme == 3), localize_ko("A silent note will be added to extend the length of the song."))
	...
	if (language != 1) draw_text_dynamic(x1 + 252, y1 + 345, localize_ko("Times to loop:"))
	...
	if (language != 1) {if (cursong.loopmax = 0) draw_text_dynamic(x1 + 360, y1 + 345, localize_ko("(infinite)"))}
	...
	if (draw_button2(x1 + 430 - 72, y1 + 386, 72, condstr(language != 1, localize_ko("OK"), "确定")) && (windowopen = 1 || theme != 3)) {
