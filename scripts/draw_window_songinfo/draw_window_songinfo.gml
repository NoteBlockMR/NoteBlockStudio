function draw_window_songinfo() {
	...
	if (language != 1) {if (str[0] = "") str[0] = localize_ko("Untitled song")}
	...
	w1 = string_width_dynamic(localize_ko("Created by "))
	w3 = string_width_dynamic(localize_ko("Originally created by "))
	draw_theme_font(font_main)
	w5 = string_width_dynamic(localize_ko("Imported from "))
	} else {
	draw_theme_font(font_info_med)
	...
	if (songs[song].song_author != "") {
	    yy += 32
	    if (language != 1) draw_text_dynamic(x1 + floor(w / 2 - w2 / 2), yy, localize_ko("Created by "))
	...
	if (songs[song].song_orauthor != "") {
	    ...
	    draw_theme_font(font_info_med)
	    draw_theme_color()
	    if (language != 1) draw_text_dynamic(x1 + floor(w / 2 - w4 / 2), yy, localize_ko("Originally created by "))
	...
	if (songs[song].song_midi != "") {
	    draw_theme_font(font_main)
		if (language != 1) a = string_width_dynamic(localize_ko("Imported from "))
		...
		draw_theme_font(font_main)
		if (language != 1) draw_text_dynamic(x1 + floor(w / 2 - b / 2), y1 + h - 34 - 32 * (window = w_songinfoedit), localize_ko("Imported from "))
		else draw_text_dynamic(x1 + floor(w / 2 - b / 2), y1 + h - 34 - 32 * (window = w_songinfoedit), "导入自：")
	}
	draw_theme_font(font_main)
	if (language != 1) draw_text_dynamic(x1 + floor(w / 2), y1 + h - 20 - 32 * (window = w_songinfoedit), condstr(window = w_songinfo, localize_ko("Click anywhere to dismiss")) + condstr(window = w_songinfoedit, localize_ko("This message is shown when the file is opened.")))
	...
	if (window = w_songinfoedit) {
		if (language != 1) {
	    if (draw_button2(x1 + floor(w / 2) - 70, y1 + h - 30, 70, localize_ko("Edit"), false, true)) window = w_properties
	    if (draw_button2(x1 + floor(w / 2) + 5, y1 + h - 30, 70, localize_ko("OK")) && (windowopen = 1 || theme != 3)) windowclose = 1
		} else {
