function draw_window_macro_setpanning() {
	...
	if (language != 1) draw_text_dynamic(x1 + 8, y1 + 8, localize_ko("Set note panning"))
	...
	if (language != 1) draw_areaheader(x1 + 10, y1 + 53, 120, 35, localize_ko("Panning"))
	else draw_areaheader(x1 + 10, y1 + 53, 120, 35, "声道")
	setpan = median(-100, draw_dragvalue(12, x1 + 55, y1 + 65, setpan, 0.1), 100)

	draw_theme_color()
	if (draw_button2(x1 + 10, y1 + 98, 60, localize_ko("OK"))) {
		windowalpha = 0
		windowclose = 0
		windowopen = 0
		window = 0
