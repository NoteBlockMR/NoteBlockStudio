function warn_working_directory_path(path) {
	...
	message(
		condstr(
			language != 1,
			localize_ko("GameMaker redirects all file access in Note Block Studio's program folder to its user-data folder. The selected file will therefore be loaded from or saved to this folder instead:\n\n") + game_save_id + localize_ko("\n\nTo access the file you intended, choose a location outside Note Block Studio's program folder."),
		...
		condstr(language != 1, localize_ko("Program folder redirected"), "程序目录已重定向")
	)
}
