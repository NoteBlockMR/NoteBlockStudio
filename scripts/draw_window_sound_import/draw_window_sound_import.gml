function draw_window_sound_import() {
	...
	if (draw_button2(x1, y1, 72, ((language != 1) ? localize_ko("Change") : "更改"), false, true)) {
		var fn = string(get_save_filename_ext("",
			((language != 1) ? localize_ko("Select Minecraft installation directory") : "选择Minecraft安装目录"),
			mc_install_path,
			((language != 1) ? "Minecraft installation directory" : "Minecraft安装目录")));
	...
	if (draw_button2(x1, y1, 86, ((language != 1) ? localize_ko("Get sounds") : "获取音效"), is_locked, false)) {
	...
	if (draw_button2(x1, y1, 120, ((language != 1) ? localize_ko("Instrument settings...") : "音色设置......"), false, true)) {
	...
	if (draw_button2(x1, y1, 72, ((language != 1) ? localize_ko("OK") : "确定"), false, false)) {
