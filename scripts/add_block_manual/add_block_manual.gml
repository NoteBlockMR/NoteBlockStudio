function add_block_manual(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	...
	if ((key < 33  || key > 57) && warning_octaves = 0) {
	    if (language != 1) message(localize_ko("NOTE: Since this block is outside the 2 octave limit, you won't be able to import it into Minecraft without a resource pack.\n\nThis warning will never be shown again."), localize_ko("Warning"))
	    else message("注意：由于此方块在 2 八度的范围外，您将无法不使用资源包将该歌曲导入进 Minecraft 中。\n此消息将不再提示。", "警告")
	    warning_octaves = 1
		save_settings()
	} else if (ins.user && warning_instrument = 0) {
	    if (language != 1) message(localize_ko("NOTE: Since this block has a custom instrument, you won't be able to import it into Minecraft without mods or resource packs.\n\nThis warning will never be shown again."), localize_ko("Warning"))
