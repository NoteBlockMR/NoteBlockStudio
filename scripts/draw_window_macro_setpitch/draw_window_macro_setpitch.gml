function draw_window_macro_setpitch() {
	...
	if (language != 1) draw_text_dynamic(x1 + 8, y1 + 8, localize_ko("Set note pitch"))
	...
	if (language != 1) draw_areaheader(x1 + 10, y1 + 53, 120, 35, localize_ko("Pitch"))
	else draw_areaheader(x1 + 10, y1 + 53, 120, 35, "音高")
	setpit = median(-1200, draw_dragvalue(18, x1 + 55, y1 + 65, setpit, 0.1), 1200)

	draw_theme_color()
	if (language != 1) {if (draw_checkbox(x1 + 15, y1 + 80, addpitch, localize_ko("Add mode"), localize_ko("Add the pitch amount to each note,\ninstead of setting it."), 0, 1)) addpitch = !addpitch}
