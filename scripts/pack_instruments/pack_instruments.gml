...
function pack_instruments() {
	...
	if (language != 1) fn = string(get_save_filename_ext(localize_ko("ZIP archive (*.zip)|*.zip"), condstr(filename == "", "", filename_change_ext(filename, "") + " - ") + "Instruments.zip", "", localize_ko("Pack instruments to ZIP file")));
	else fn = string(get_save_filename_ext(localize_ko("ZIP archive (*.zip)|*.zip"), condstr(filename == "", "", filename_change_ext(filename, "") + " - ") + "Instruments.zip", "", "导出音色至 ZIP 文件"));
	if (fn = "") return 0;
