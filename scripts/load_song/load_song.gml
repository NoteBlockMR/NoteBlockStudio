function load_song() {
	...
	if (!backup && fn = "") {
	    ...
	    fn = string(get_open_filename_ext(localize_ko("Note Block Songs (*.nbs)|*.nbs|Zipped Files (*.zip)|*.zip|MIDI Sequences (*.mid)|*.mid;*.midi|Minecraft Schematics (*.schematic)|*.schematic"), "", songfolder, condstr(language != 1, localize_ko("Load song"), "打开歌曲")))
	}
