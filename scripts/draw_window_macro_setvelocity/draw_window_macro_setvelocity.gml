function draw_window_macro_setvelocity() {
	...
	if (language != 1) draw_text_dynamic(x1 + 8, y1 + 8, localize_ko("Set note velocity"))
	...
	if (language != 1) draw_areaheader(x1 + 10, y1 + 53, 120, 35, localize_ko("Velocity"))
	else draw_areaheader(x1 + 10, y1 + 53, 120, 35, "音量")
	setvel = median(0, draw_dragvalue(10, x1 + 55, y1 + 65, setvel, 0.1), 100 + (percentvel * 900))

	draw_theme_color()
	if (language != 1) {if (draw_checkbox(x1 + 15, y1 + 80, percentvel, localize_ko("Percent"), localize_ko("Apply the velocity as a percentage\nof each note's current velocity."), 0, 1)) percentvel = !percentvel}
