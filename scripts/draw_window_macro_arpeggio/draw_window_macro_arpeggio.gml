function draw_window_macro_arpeggio() {
	...
	draw_text_dynamic(x1 + 8, y1 + 8, localize_ko("Arpeggio"))
	pattern = ""
	...
	if (language != 1) draw_areaheader(x1 + 10, y1 + 43, 120, 35, localize_ko("Pattern"))
	else draw_areaheader(x1 + 10, y1 + 43, 120, 35, "规律")

	if (language != 1) pattern = draw_textarea(57, x1 + 15, y1 + 50, 113, 25, string(pattern), localize_ko("Must separate relative keys with pipes.")) 
	...
	if (draw_button2(x1 + 70, y1 + 98, 60, condstr(language !=1, localize_ko("Cancel"), "取消"), false, true) && (windowopen = 1 || theme != 3)) {windowclose = 1}
