function draw_window_instruments() {
	...
	if (language != 1) {
	if (draw_button2(x1 + 455, y1 + 9, 80, localize_ko("Import"), 0, 1)) load_instruments("")
	if (draw_button2(x1 + 455, y1 + 36, 80, localize_ko("Open Folder"), 0, 1)) open_url(sounds_directory)
	// Tabs
	...
	if (language != 1) {if (draw_button2(x1 + 12, y1 + 318, 86, localize_ko("Export sounds"), (user_instruments == 0 || sounds == 0))) pack_instruments()}
	else {if (draw_button2(x1 + 12, y1 + 318, 86, "导出音色", (user_instruments == 0 || sounds == 0))) pack_instruments()}
	c = 0
	if (draw_button2(x1 + 110, y1 + 318, 80, condstr(language != 1, localize_ko("Add"), "添加"), user_instruments >= 240) && wmenu = 0) {
	    changed = true
	...
	if (language != 1) {
	if (draw_button2(x1 + 194, y1 + 318, 80, localize_ko("Remove"), userselect < 0) && wmenu = 0) {
	...
	if (draw_button2(x1 + 278, y1 + 318, 80, localize_ko("Shift up"), (userselect < 0) || (user_instruments <= 1) || (insselect == first_custom_index)) && wmenu = 0) {
		insselect -= 1
		instrument_swap(userselect, instrument_list[| insselect])
		c = 1
	}
	if (draw_button2(x1 + 362, y1 + 318, 80, localize_ko("Shift down"), (userselect < 0) || (user_instruments <= 1) || (insselect == ds_list_size(instrument_list) - 1) && wmenu = 0)) {
		insselect += 1
		instrument_swap(userselect, instrument_list[| insselect])
		c = 1
	}
	if (draw_button2(x1 + 456, y1 + 318, 80, localize_ko("OK")) && wmenu = 0 && (windowopen = 1 || theme != 3)) {
		...
		if (save_version < 5 && user_instruments > 18) {
			show_message(localize_ko("This song contains more than 18 instruments and cannot be saved in version ") + string(save_version) + localize_ko(". The save version will be changed to ") + string(nbs_version) + ".")
