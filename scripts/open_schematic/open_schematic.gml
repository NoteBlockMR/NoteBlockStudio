function open_schematic(argument0) {
	...
	if (fn = "" || !file_exists_lib(fn)) fn = string(get_open_filename_ext(localize_ko("Minecraft Schematics (*.schematic)|*.schematic"), "", "", localize_ko("Import from Schematic")))
	...
	with (create(obj_dummy)) {
	    ...
	    nb_z = -1
	    buffer_read_byte() // 10
	    if (buffer_read_string_short_be() != localize_ko("Schematic")) { //"Schematic"
	        message(localize_ko("Failed to load Schematic.\n\nERROR: Not a Schematic file."), localize_ko("Error"))
	        window = 0
	    ...
	    if (sch_width <= 0 || sch_length <= 0 || sch_height <= 0) {
	        message(localize_ko("Failed to load Schematic.\n\nERROR: Invalid size."), localize_ko("Error"))
	        window = 0
	        buffer_delete(buffer)
	        instance_destroy()
	        return 0
	    ...
	    if (blockspos < 0) {
	        message(localize_ko("Failed to load Schematic.\n\nERROR: No Blocks array found."), localize_ko("Error"))
	        window = 0
	        buffer_delete(buffer)
	        instance_destroy()
	        return 0
	    ...
	    if (datapos < 0) {
	        message(localize_ko("Failed to load Schematic.\n\nERROR: No Data array found."), localize_ko("Error"))
	        window = 0
	        buffer_delete(buffer)
	        instance_destroy()
	        return 0
	    ...
	    if (noteblocks = 0) {
	        message(localize_ko("Couldn't find any note blocks in the Schematic!"), localize_ko("Error")) 
	        window = 0
	        buffer_delete(buffer)
	        instance_destroy()
	        return 0
	    ...
	    if (inputam = 0) {
	        message(localize_ko("The Schematic did not include any supported inputs.\nSupported inputs are:\n\n* Buttons\n* Levers"), localize_ko("Error"))
	        window = 0
	        instance_destroy()
	        return 0
	    }
	    ...
	    if (queuenoteblocks = 0) {
	        message(localize_ko("None of the note blocks in the Schematic could be added. This could be to none of them being connected to any inputs (buttons or levers)."), localize_ko("Error"))
	        instance_destroy()
	        return 0
	    }
	    if (inputam > 1) message(localize_ko("The Schematic contained more than one input.\n\nAs a result, the songs of the inputs have been put after each other."), localize_ko("Schematic import"))
