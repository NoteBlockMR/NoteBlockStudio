function draw_window_set_tempo() {
	...
	draw_set_alpha(1)
	if (language != 1) input = draw_inputbox(64, xx, 57 + song_tab_offset, w, input, localize_ko("Enter the new tempo (in ") + condstr(use_bpm, localize_ko("beats per minute"), localize_ko("ticks per second")) + ")", (0.3 + 0.3 * !fdark) * (acrylic && wpaperexist) + (!acrylic || !wpaperexist))
	...
	if (window == 0) {
		...
		if (tutorial_tempobox == 1) {
			if (language != 1) set_msg(localize_ko("Good! Right-clicking the box\nwill show some handy options."), 7.0, 228, 118 + song_tab_offset)
