function draw_window_set_tempo() {
	...
	draw_set_alpha(1)
	if (language != 1) input = draw_inputbox(64, xx, 57, w, input, localize_ko("Enter the new tempo (in ") + condstr(use_bpm, localize_ko("beats per minute"), localize_ko("ticks per second")) + ")", (0.3 + 0.3 * !fdark) * (acrylic && wpaperexist) + (!acrylic || !wpaperexist))
