...
function find_asset_indexes() {
	...
	if (!directory_exists(mc_install_path)) {
		if (language != 1) show_message(localize_ko("No Minecraft installation was found\nat the selected location!"));
...
function load_asset_index(copy = false) {
	...
	if (!file_exists(asset_index_path)) {
		if (language != 1) show_message(localize_ko("The file for the specified asset index could not be found!"))
